...
| 应用更新 | 启动检查 GitHub Releases、手动检查、一键下载、安装和重启 |
...
Windows 发行版每次启动会在后台检查一次 GitHub 正式发行版。发现新版本后，可在“设置 → 更新”中确认下载；程序会校验并解压 ZIP、保留 `data` 目录、替换应用文件后自动重启。源码运行模式不会执行自动安装。
...
- **侧边栏 Agent**：适合部署、配置、联网查询和多轮任务，可使用 workspace、SFTP 与 MCP 工具。每台服务器有独立 workspace；向 Agent 发送“初始化主机”可在其中创建或刷新 `AGENTS.md`，自动记录主机、服务、容器和端口信息，同时保留用户补充的用途与运维约定。该文件会自动加入此主机后续 Agent 会话的系统提示。用户明确要求时，Agent 也可申请访问 workspace 的上一级共享根目录，用于复用脚本或持久保存不随服务器删除的文件。
...
- 首次连接需确认服务器 SHA-256 指纹；已保存的指纹发生变化时会再次提示核对，确认信任新指纹后可重新连接。
- 备份中的敏感数据保持加密，恢复时需要原主密码。
- 打包版数据位于程序目录下的 `data`，升级时应保留该目录。

源码运行时，数据库与 Agent 工作区分别位于 `data/webssh.db` 和 `data/workspace`。服务器独立目录位于 `data/workspace/server-{id}`；其中的 `AGENTS.md` 可由用户直接编辑，自动初始化只会替换 `coshell:host-info` 标记之间的内容。共享文件可直接存放在 `data/workspace` 下，Agent 每次任务访问共享根目录前都需要用户明确批准，且“完全访问”模式不会跳过这项授权。本项目按单用户本地应用设计；如需远程开放访问，必须另行配置 HTTPS、身份认证、访问控制和审计。
...
构建结果位于 `dist\CoShell`，同时会生成可直接上传到 GitHub Release 的 `dist\CoShell-v<版本>-windows-x64-portable.zip`。这是 PyInstaller 目录版，发布时必须将 `CoShell.exe` 与 `_internal` 整体打包；自动更新会从 Release assets 中选择 Windows ZIP 包。
