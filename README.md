...
- **侧边栏 Agent**：适合部署、配置、联网查询和多轮任务，可使用 workspace、SFTP 与 MCP 工具。每台服务器有独立 workspace；用户明确要求时，Agent 也可申请访问其上一级共享根目录，用于复用脚本或持久保存不随服务器删除的文件。
...
源码运行时，数据库与 Agent 工作区分别位于 `data/webssh.db` 和 `data/workspace`。服务器独立目录位于 `data/workspace/server-{id}`；共享文件可直接存放在 `data/workspace` 下，Agent 每次任务访问共享根目录前都需要用户明确批准，且“完全访问”模式不会跳过这项授权。本项目按单用户本地应用设计；如需远程开放访问，必须另行配置 HTTPS、身份认证、访问控制和审计。
