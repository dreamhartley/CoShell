...
完成后运行 `dist\LightSSHTerminal\LightSSHTerminal.exe`，目标电脑无需单独安装 Python。打包版数据保存在 `%LOCALAPPDATA%\LightSSHTerminal`，升级或替换程序不会丢失配置。

## 数据与安全

- 桌面应用仅在 `127.0.0.1` 随机端口监听；源码运行时数据保存在 `data/webssh.db`。
- Agent 的本机工作目录位于应用数据目录中的 `workspace`：源码运行时为 `data/workspace`，打包版为 `%LOCALAPPDATA%\LightSSHTerminal\workspace`。每台已保存服务器使用稳定的 `server-<ID>` 独立子目录，临时连接按连接身份隔离；Agent 无法通过本地工具越过当前服务器的目录。删除服务器时可选择一并删除对应 workspace 文件。
...
连接 SSH 后，在终端输入自然语言任务：

```text
/agent 帮我在 VPS 上安装 ufw 并放行 22、443 端口
/agent 把 https://github.com/example/project 部署到这台服务器
```

在终端中使用 `/agent` 时，会自动附带当前终端最近的输出上下文，因此遇到报错时可直接输入 `/agent 帮我看一下这个报错`。左侧边栏的 Agent 聊天不会自动附带终端内容，仍用于独立描述和处理任务。

Agent 会先阅读你提供的项目页或文档，随后在当前 SSH 连接上通过独立命令通道检查环境、执行部署并验证结果。单次任务最多 30 轮，单条命令最长可运行 15 分钟；达到上限后可用 `/agent 继续刚才的任务` 沿用上下文继续。`/agent` 也可用于普通问答，回答中的 Markdown 会转换为终端样式显示。

每次请求的系统提示会包含当前本地日期和时区。Agent 可通过 `workspace_list`、`workspace_read`、`workspace_write` 管理 `workspace` 中的 UTF-8 文本文件，并通过 `sftp_transfer` 在 `workspace` 与当前 SSH 主机之间上传或下载单个文件。本地路径必须相对于 `workspace`；文本工具限制为 256 KiB，SFTP 单文件限制为 512 MiB，覆盖已有文件必须显式确认。
