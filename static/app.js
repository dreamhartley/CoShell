  ...
  for(const char of data){
    if(tab.agentBuffer!==null){
      ...
      if(char==='\r'||char==='\n'){
        ...
        if(captured==='/agent'||captured.startsWith('/agent ')){tab.term.write('\r\n');runAgent(tab,captured.slice(6).trim())}else{eraseLocalInput(tab.term,captured);rawTerminalInput(tab,captured+'\r')}continue
      }
...
function showReconnectPrompt(tab){if(tab.localPromptShown)return;tab.localPromptShown=true;tab.localInput='';tab.term.write('\x1b[?1049l\r\n\x1b[90m[连接已断开，输入 /reconnect 重新连接]\x1b[0m\r\n\x1b[36mlocal> \x1b[0m')}
...
$('#ssh-key-import-toggle').onclick=()=>$('#ssh-key-import-form').classList.remove('hidden');
...
function newTerminal(tabData={}){
  ...
  const tab={id,title:tabData.title||'新终端',server_id:tabData.server_id??null,last_path:tabData.last_path||'.',position:state.tabs.length,status:'idle',term,fit,host,ws:null,sessionId:null,localInput:'',localPromptShown:false,typedLine:'',agentBuffer:null,agentBusy:false,agentBusyNotice:false};
...
function renderTabs(){
  ...
  state.tabs.forEach(tab=>{const el=document.createElement('div');el.className='terminal-tab'+(tab.id===state.activeId?' active':'');el.dataset.id=tab.id;el.draggable=true;
    ...
    el.append(dot,title,close);el.onclick=()=>activateTab(tab.id);el.oncontextmenu=e=>{e.preventDefault();const action=prompt('标签操作：reconnect / rename / close');if(action==='reconnect'){activateTab(tab.id);tab.server_id?connectTab(tab,{server_id:tab.server_id}):openConnect({name:tab.title})}else if(action==='rename'){const name=prompt('标签名称',tab.title);if(name?.trim()){tab.title=name.trim();renderTabs();saveTabs()}}else if(action==='close')closeTab(tab)};title.ondblclick=e=>{e.stopPropagation();const name=prompt('标签名称',tab.title);if(name?.trim()){tab.title=name.trim();renderTabs();saveTabs()}};close.onclick=e=>{e.stopPropagation();closeTab(tab)};
...
function closeTab(tab){
  if(tab.status==='connected'&&!confirm(`关闭 ${tab.title} 并断开 SSH？`))return;
...
function connectTab(tab,connection){
  const reconnecting=tab.localPromptShown;if(tab.ws?.readyState===WebSocket.OPEN)tab.ws.close();tab.status='connecting';tab.localInput='';tab.typedLine='';tab.agentBuffer=null;tab.localPromptShown=false;renderTabs();tab.term.writeln(`\r\n\x1b[33m${reconnecting?'正在重新连接':'正在连接'}…\x1b[0m`);
  ...
  ws.onmessage=e=>{if(tab.ws!==ws)return;const m=JSON.parse(e.data);if(m.type==='connected'){tab.status='connected';tab.sessionId=m.session_id;tab.term.writeln('\x1b[32m已连接\x1b[0m');renderTabs();scheduleTerminalFit(tab,true);loadSftp();loadSystemInfo();loadServers()}
    else if(m.type==='output'){const bytes=Uint8Array.from(atob(m.data),c=>c.charCodeAt(0));tab.term.write(bytes)}
    else if(m.type==='host_key')showHostKeyDialog(ws,m)
    else if(m.type==='error'){tab.status='error';tab.term.writeln(`\r\n\x1b[31m${m.message}\x1b[0m`);renderTabs();showReconnectPrompt(tab);toast(m.message,true)}else if(m.type==='disconnected'){tab.status='idle';tab.sessionId=null;renderTabs();showReconnectPrompt(tab);loadSystemInfo()}};
  ws.onerror=()=>{if(tab.ws!==ws)return;tab.status='error';renderTabs();showReconnectPrompt(tab)};ws.onclose=()=>{if(pendingHostKey?.ws===ws){pendingHostKey=null;if($('#host-key-dialog').open)$('#host-key-dialog').close()}if(tab.ws!==ws)return;if(tab.status==='connected'||tab.status==='connecting'){tab.status='idle';tab.sessionId=null;renderTabs();showReconnectPrompt(tab);loadSftp();loadSystemInfo()}};
}
...
function renderShortcuts(){const q=$('#shortcut-search').value.toLowerCase(),root=$('#shortcut-list');root.replaceChildren();state.shortcuts.filter(s=>`${s.name} ${s.command} ${s.group_name}`.toLowerCase().includes(q)).forEach(s=>{const card=document.createElement('div');card.className='card';card.innerHTML=`<div class="card-title">${esc(s.name)}</div><div class="card-sub">${esc(s.group_name||'未分组')} · ${esc(s.command.replace(/\n/g,' ↵ '))}</div><div class="card-actions"><button class="fill">填入</button><button class="run">执行</button><button class="edit">编辑</button><button class="delete danger">删除</button></div>`;$('.fill',card).onclick=()=>sendShortcut(s,false);$('.run',card).onclick=()=>sendShortcut(s,true);$('.edit',card).onclick=()=>editShortcut(s);$('.delete',card).onclick=async()=>{if(confirm(`删除“${s.name}”？`)){await api(`/api/shortcuts/${s.id}`,{method:'DELETE'});loadShortcuts()}};root.append(card)});if(!root.children.length)root.innerHTML='<div class="empty">暂无快捷指令</div>'}
function sendShortcut(s,run){const tab=activeTab();if(!tab||tab.status!=='connected')return toast('请先连接并选择一个终端',true);if(run&&s.command.includes('\n')&&!confirm('执行多行脚本？'))return;sendTerminalInput(tab,s.command+(run?'\n':''));tab.term.focus()}
...
$('#batch-btn').onclick=async()=>{const items=[...state.selectedFiles],tab=activeTab();if(!items.length||!tab)return;const action=prompt('输入批量操作：move / copy / delete');if(!action)return;try{if(action==='delete'){if(!confirm(`递归删除选中的 ${items.length} 项？`))return;for(const item of items)await api('/api/sftp/delete',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,path:joinRemote(tab.last_path,item.name)})})}else if(action==='move'||action==='copy'){const target=prompt('目标目录完整路径');if(!target)return;for(const item of items)await api(`/api/sftp/${action}`,{method:'POST',body:JSON.stringify({session_id:tab.sessionId,source:joinRemote(tab.last_path,item.name),destination:joinRemote(target,item.name),overwrite:false})})}else return toast('未知操作',true);toast('批量操作完成');loadSftp()}catch(err){toast(err.message,true)}};
...
async function fileAction(item){const action=prompt(`操作 ${item.name}\n输入：rename / move / copy / delete${item.is_dir?'':' / download'}`);if(!action)return;const tab=activeTab(),source=joinRemote(tab.last_path,item.name);try{if(action==='download'&&!item.is_dir)return downloadFile(item.name);if(action==='delete'){if(!confirm(`递归删除“${item.name}”？`))return;await api('/api/sftp/delete',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,path:source})})}else if(['rename','move','copy'].includes(action)){const dst=prompt('目标完整路径',source);if(!dst||dst===source)return;await api(`/api/sftp/${action==='copy'?'copy':'move'}`,{method:'POST',body:JSON.stringify({session_id:tab.sessionId,source,destination:dst,overwrite:false})})}else return toast('未知操作',true);loadSftp();toast('操作完成')}catch(err){if(err.message.includes('exist')&&confirm('目标已存在，是否覆盖？'))toast('请重新操作并使用其他目标路径',true);else toast(err.message,true)}}
...
function showFileMenu(event,item){const tab=activeTab(),items=chosenItems(item),paths=items.map(x=>joinRemote(tab.last_path,x.name));showContextMenu(event,[
  ...
  {label:'重命名',disabled:items.length!==1,run:async()=>{const name=prompt('新名称',item.name);if(!name||name===item.name)return;await api('/api/sftp/move',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,source:paths[0],destination:joinRemote(tab.last_path,name),overwrite:false})});loadSftp()}},
  null,
  {label:items.length>1?`删除 ${items.length} 项`:'删除',run:async()=>{if(!confirm(`确定递归删除选中的 ${items.length} 项？`))return;for(const path of paths)await api('/api/sftp/delete',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,path})});loadSftp()}},
  {label:'复制文件路径',disabled:items.length!==1,run:()=>copyText(paths[0])}
])}
async function pasteRemote(destination){const tab=activeTab(),clip=state.remoteClipboard;if(!clip)return;if(clip.sessionId!==tab.sessionId)throw new Error('当前版本仅支持在同一个 SSH 会话内粘贴');for(const item of clip.items){const target=joinRemote(destination,item.name);try{await api('/api/sftp/copy',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,source:item.path,destination:target,overwrite:false})})}catch(err){if(!err.message.includes('存在')||!confirm(`${item.name} 已存在，是否覆盖？`))throw err;await api('/api/sftp/copy',{method:'POST',body:JSON.stringify({session_id:tab.sessionId,source:item.path,destination:target,overwrite:true})})}}toast('粘贴完成');loadSftp()}
...
async function uploadFiles(files) {
  ...
  for (const task of tasks) {
    try {
      ...
      if (err.message.includes('存在') && confirm(`${task.file.name} 已存在，是否覆盖？`)) {
        try {
...
$('#sidebar-toggle').onclick=()=>{$('#sidebar').classList.toggle('collapsed');scheduleTerminalFit(activeTab())};let resizing=false;$('#resizer').onmousedown=()=>resizing=true;window.addEventListener('mousemove',e=>{if(resizing){const w=Math.max(240,Math.min(innerWidth*.55,e.clientX));$('#sidebar').style.width=`${w}px`;localStorage.setItem('webssh-sidebar-width',w)}});window.addEventListener('mouseup',()=>{if(resizing)scheduleTerminalFit(activeTab());resizing=false});const sw=localStorage.getItem('webssh-sidebar-width');if(sw)$('#sidebar').style.width=`${sw}px`;
